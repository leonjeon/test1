...
public class TestGenerics1 {
	...
	public static void main(String[] args) {
		...
		System.out.println(list); //저장 순서대로 저장된 것 확인용
		
		//저장 후에 하나씩 꺼내서 사용하려고 할 때, 기본적으로 Object 타입으로 추출된다.
		for(Object obj: list) {
			//Object 타입의 레퍼런스로는 Object 클래스가 가진 메소드나 필드만 처리할 수 있다.
			//toString(), equals(), clone(), hashCode() 메소드만 사용할 수 있는 상태임
			//후손클래스들이 오버라이딩한 경우에는 후손쪽의 오버라이딩된 부모메소드가 자동 연결 되면서 실행되게 됨
			...
			// 타입이 다른 객체를 다른 클래스형으로 형변환하게되면 에러 발생한다.
			//=> 레퍼런스가 참조하는 객체의 클래스타입을 확인하고 형변환해야한다. (DownCasting)
			//레퍼런스가 참조하는 객체의 클래스타입을 확인할 때 instanceof 연산자 사용한다.
