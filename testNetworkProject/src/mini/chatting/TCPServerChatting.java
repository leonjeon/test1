...
public class TCPServerChatting {
	...
	private static void clientAccess(ServerSocket server) {
		...
		try {
			//클라이언트가 접속 요청하면, 요청 수락하면서 클라이언트 정보 저장한 소켓을 만든다.
			Socket client = server.accept();
			String clientIP = client.getInetAddress().getHostAddress(); //접속 요청한 클라이언트 ip주소 추출함
			...
			try {
				//메세지 주고 받기 : 서버가 먼저 닫고(읽어드림) 답을 보내는 순서임
				...
				System.out.println(clientIP + " : diconnected....");
				pw.close();
				br.close();
				client.close();
